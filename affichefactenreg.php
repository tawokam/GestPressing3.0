...
$nom="SELECT facture.code,facture.id_client,commande.utilisateur,commande.id_client,commande.code,commande.date_depot,commande.date_retrait,client.id_client,client.nom_cl FROM commande inner join facture on commande.code=facture.code inner join client on commande.id_client=client.id_client where facture.code='$codefact' AND facture.agence='$agence' AND facture.id_client='$idcl' group by facture.code,facture.id_client,commande.code,commande.id_client";
        if($nome=$connec->query($nom)){
            while($date=$nome->fetch()){
                ?>
                <div style="text-align:center;font-size:14px">
                <table  style="width:100%;text-align:center;font-size:14px"><tr><th>Doit Mr/Mme</th><th>Habits déposés le</th><th>Habits a rétiré le</th><th>Facture N°</th></tr>
                <tr><td><?php echo $date['nom_cl']?></td><td><?php echo $date['date_depot']?></td><td><?php echo $date['date_retrait']?></td><td><?php echo $date['code'] ?></td></tr>
                </table></div>
        ...
        $so="SELECT * from facture where code='$codefact' AND agence='$agence'";
        if($som=$connec->query($so)){
            while($some=$som->fetch()){

                ?>
                <div>Totaux:<?php echo $some['monttotal'] ?>Fcfa&nbsp; &nbsp; Montant net a payer: <?php echo $some['monttotal'] ?>Fcfa &nbsp; &nbsp;Montant versé: <?php echo $some['avance']?>Fcfa  &nbsp; &nbsp; Reste:<?php echo $some['reste'] ?>Fcfa</div>
                <?php

            }
        }
             //affichage du nombre d'habit
             $affinbre="SELECT *,sum(quantite_cmd) as qte FROM commande where commande.code='$codefact' AND agence='$agence'";
             if($affin=$connec->query($affinbre)){
                 while($anbre=$affin->fetch()){
                     echo '<div style="font-weight:bold">Nombre de vetements déposé: '.$anbre['qte'].'</div>';
                 }
             }
